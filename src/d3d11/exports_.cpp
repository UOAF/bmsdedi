                ...
                _Out_opt_ ID3D11DeviceContext** ppImmediateContext )
{
    ...
    auto result = func(pAdapter, DriverType, Software, Flags, pFeatureLevels,
                       FeatureLevels, SDKVersion, ppDevice, pFeatureLevel,
                       &real_device_context);
    if (SUCCEEDED(result) and (real_device_context != nullptr))
    {
        *ppImmediateContext =
            new D3D11DeviceContextWrapper(real_device_context);
    }
    return result;
}
